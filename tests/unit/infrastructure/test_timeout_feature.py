...
from app.models.models import AccountPermission, Capability, ConnectionAccount, McpService, Service
...
def test_data(db):
    """Create test data"""
    # Create MCP Service
    mcp_service = McpService(name="Test MCP Service", identifier="test-mcp", routing_type="subdomain")
    ...
    # Create Account
    account = ConnectionAccount(name="Test Account", bearer_token="test_token_123")
    ...
    db.session.add_all([capability_with_custom_timeout, capability_with_default_timeout, capability_with_no_timeout])
    db.session.flush()

    # Add permissions
    for cap in [capability_with_custom_timeout, capability_with_default_timeout, capability_with_no_timeout]:
        permission = AccountPermission(account_id=account.id, capability_id=cap.id)
...
def test_capability_creation_with_timeout(client, db):
    ...
    mcp_response = client.post(
        "/api/mcp-services", json={"name": "Test Service", "identifier": "test-service", "routing_type": "subdomain"}
    )
    assert mcp_response.status_code == 201
    mcp_service_id = mcp_response.json["id"]

    app_response = client.post(
        f"/api/mcp-services/{mcp_service_id}/apps",
        json={"name": "Test App", "service_type": "api", "mcp_url": "https://api.example.com"},
    )
    ...
    response = client.post(
        f"/api/apps/{app_id}/capabilities",
        json={
...
def test_capability_update_timeout(client, db, test_data):
    ...
    # Update timeout
    response = client.put(
        f"/api/capabilities/{capability.id}",
        json={
