...
from app.models.models import AccountPermission, ConnectionAccount, McpService
...
class TestAccountPermissionAPI:
    ...
    def test_get_account_permissions(self, auth_client, db, sample_account, sample_capability):
        """アカウントの権限一覧取得"""
        permission = AccountPermission(account_id=sample_account.id, capability_id=sample_capability.id)
    ...
    def test_grant_capability_permission(self, auth_client, db, sample_account, sample_capability):
        ...
        response = auth_client.post(
            f"/api/accounts/{sample_account.id}/permissions", json={"capability_id": sample_capability.id}
        )
        ...
        permission = AccountPermission.query.filter_by(
            account_id=sample_account.id, capability_id=sample_capability.id
        ).first()
    ...
    def test_grant_app_permission(self, auth_client, db, sample_account, sample_service):
        ...
        response = auth_client.post(
            f"/api/accounts/{sample_account.id}/permissions", json={"app_id": sample_service.id}
        )
        assert response.status_code == 201
        data = response.get_json()
        assert data["account_id"] == sample_account.id
    ...
    def test_grant_mcp_service_permission(self, auth_client, db, sample_account):
        ...
        mcp_service = McpService(
            identifier="test-mcp", name="Test MCP Service", routing_type="subdomain", access_control="restricted"
        )
        db.session.add(mcp_service)
        db.session.commit()

        response = auth_client.post(
            f"/api/accounts/{sample_account.id}/permissions", json={"mcp_service_id": mcp_service.id}
        )
        assert response.status_code == 201
        data = response.get_json()
        assert data["account_id"] == sample_account.id
    ...
    def test_grant_permission_invalid_multiple_levels(
        self, auth_client, db, sample_account, sample_service, sample_capability
    ):
        """複数レベル同時指定はエラー"""
        response = auth_client.post(
            f"/api/accounts/{sample_account.id}/permissions",
            json={"app_id": sample_service.id, "capability_id": sample_capability.id},
        )
    ...
    def test_grant_permission_invalid_no_level(self, auth_client, db, sample_account):
        """レベル未指定はエラー"""
        response = auth_client.post(f"/api/accounts/{sample_account.id}/permissions", json={})
        assert response.status_code == 400
...
class TestCapabilityPermissionsAPI:
    ...
    def test_get_capability_permissions(self, auth_client, db, sample_capability):
        ...
        account1 = ConnectionAccount(name="Account 1", bearer_token=f"test_token_{secrets.token_hex(16)}")
        account2 = ConnectionAccount(name="Account 2", bearer_token=f"test_token_{secrets.token_hex(16)}")
        db.session.add_all([account1, account2])
        db.session.commit()

        # Grant permission to account1
        permission = AccountPermission(account_id=account1.id, capability_id=sample_capability.id)
        db.session.add(permission)
        db.session.commit()
    ...
    def test_update_capability_permissions(self, auth_client, db, sample_capability):
        ...
        account1 = ConnectionAccount(name="Account 1", bearer_token=f"test_token_{secrets.token_hex(16)}")
        account2 = ConnectionAccount(name="Account 2", bearer_token=f"test_token_{secrets.token_hex(16)}")
        account3 = ConnectionAccount(name="Account 3", bearer_token=f"test_token_{secrets.token_hex(16)}")
        ...
        response = auth_client.put(
            f"/api/capabilities/{sample_capability.id}/permissions", json={"account_ids": [account2.id, account3.id]}
        )
        assert response.status_code == 200

        # Verify
        permissions = AccountPermission.query.filter_by(capability_id=sample_capability.id).all()
...
class TestMcpServicePermissionsAPI:
    ...
    def test_get_mcp_service_permissions(self, auth_client, db, sample_account):
        """MCP Serviceの権限一覧取得"""
        mcp_service = McpService(
            identifier="test-mcp", name="Test MCP", routing_type="subdomain", access_control="restricted"
        )
        db.session.add(mcp_service)
        db.session.commit()

        permission = AccountPermission(account_id=sample_account.id, mcp_service_id=mcp_service.id)
        db.session.add(permission)
        db.session.commit()
    ...
    def test_grant_mcp_service_permission_via_service_endpoint(self, auth_client, db, sample_account):
        ...
        mcp_service = McpService(
            identifier="test-mcp-2", name="Test MCP 2", routing_type="subdomain", access_control="restricted"
        )
        db.session.add(mcp_service)
        db.session.commit()

        response = auth_client.post(
            f"/api/mcp-services/{mcp_service.id}/permissions", json={"account_ids": [sample_account.id]}
        )
        assert response.status_code == 200

        # Verify
        permission = AccountPermission.query.filter_by(
            account_id=sample_account.id, mcp_service_id=mcp_service.id
        ).first()
        assert permission is not None
...
class TestAppPermissionsAPI:
    ...
    def test_get_app_permissions(self, auth_client, db, sample_service, sample_account):
        """Appの権限一覧取得"""
        permission = AccountPermission(account_id=sample_account.id, app_id=sample_service.id)
        db.session.add(permission)
        db.session.commit()
    ...
    def test_grant_app_permission_via_app_endpoint(self, auth_client, db, sample_service, sample_account):
        ...
        response = auth_client.post(
            f"/api/apps/{sample_service.id}/permissions", json={"account_ids": [sample_account.id]}
        )
        assert response.status_code == 200

        permission = AccountPermission.query.filter_by(account_id=sample_account.id, app_id=sample_service.id).first()
        assert permission is not None
...
class TestHierarchicalPermissions:
    ...
    def test_account_permissions_by_level(self, auth_client, db, sample_account, sample_service, sample_capability):
        """レベル別権限取得"""
        mcp_service = McpService(identifier="test-mcp", name="Test MCP", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()

        # Grant different levels
        perm_mcp = AccountPermission(account_id=sample_account.id, mcp_service_id=mcp_service.id)
        perm_app = AccountPermission(account_id=sample_account.id, app_id=sample_service.id)
        perm_cap = AccountPermission(account_id=sample_account.id, capability_id=sample_capability.id)
    ...
    def test_permission_inheritance(self, auth_client, db, sample_account, sample_service, sample_capability):
        ...
        permissions = AccountPermission.query.filter_by(account_id=sample_account.id, app_id=sample_service.id).all()
