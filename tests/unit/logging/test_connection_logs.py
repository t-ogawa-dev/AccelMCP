...
from app.models.models import ConnectionAccount, McpConnectionLog, McpService
...
class TestConnectionLogAPI:
    ...
    def test_get_connection_logs_filtered_by_success(self, auth_client, db):
        ...
        log_success = McpConnectionLog(
            mcp_method="tools/call", request_id="req-success", status_code=200, is_success=True
        )
...
class TestConnectionLogRetention:
    ...
    def test_old_logs_cleanup(self, auth_client, db):
        ...
        response = auth_client.delete("/api/connection-logs/cleanup", json={"retention_days": 90})
        assert response.status_code == 200
