...
class TestAdminSettingsModel:
    ...
    def test_create_setting(self, db):
        ...
        setting = AdminSettings(setting_key="test_key", setting_value="test_value")
    ...
    def test_setting_to_dict(self, db):
        ...
        setting = AdminSettings(setting_key="max_connections", setting_value="100")
        db.session.add(setting)
        db.session.commit()
    ...
    def test_unique_setting_key(self, db):
        ...
        setting1 = AdminSettings(setting_key="unique_key", setting_value="value1")
        db.session.add(setting1)
        db.session.commit()

        setting2 = AdminSettings(setting_key="unique_key", setting_value="value2")
...
class TestAdminSettingsAPI:
    ...
    def test_get_setting_by_key(self, auth_client, db):
        ...
        setting = AdminSettings(setting_key="specific_key", setting_value="specific_value")
        db.session.add(setting)
        db.session.commit()
    ...
    def test_create_or_update_setting(self, auth_client, db):
        ...
        payload = {"setting_key": "new_setting", "setting_value": "new_value"}
        response = auth_client.post("/api/settings", data=json.dumps(payload), content_type="application/json")
    ...
    def test_update_existing_setting(self, auth_client, db):
        ...
        setting = AdminSettings(setting_key="update_key", setting_value="old_value")
        db.session.add(setting)
        db.session.commit()

        payload = {"setting_key": "update_key", "setting_value": "new_value"}
        response = auth_client.post("/api/settings", data=json.dumps(payload), content_type="application/json")
    ...
    def test_delete_setting(self, auth_client, db):
        ...
        setting = AdminSettings(setting_key="delete_key", setting_value="delete_value")
        db.session.add(setting)
        db.session.commit()
...
class TestSecuritySettings:
    ...
    def test_login_max_attempts_setting(self, db):
        ...
        setting = AdminSettings.query.filter_by(setting_key="login_max_attempts").first()

        if not setting:
            setting = AdminSettings(setting_key="login_max_attempts", setting_value="5")
            db.session.add(setting)
            db.session.commit()
    ...
    def test_login_lock_duration_setting(self, db):
        ...
        setting = AdminSettings.query.filter_by(setting_key="login_lock_duration_minutes").first()

        if not setting:
            setting = AdminSettings(setting_key="login_lock_duration_minutes", setting_value="5")
            db.session.add(setting)
            db.session.commit()
    ...
    def test_audit_log_retention_setting(self, db):
        ...
        setting = AdminSettings.query.filter_by(setting_key="audit_log_retention_days").first()

        if not setting:
            setting = AdminSettings(setting_key="audit_log_retention_days", setting_value="365")
            db.session.add(setting)
            db.session.commit()
...
class TestLanguageSetting:
    ...
    def test_get_language_setting(self, auth_client, db):
        ...
        if not setting:
            setting = AdminSettings(setting_key="language", setting_value="ja")
            db.session.add(setting)
            db.session.commit()
    ...
    def test_update_language_setting(self, auth_client, db):
        ...
        payload = {"language": "en"}
        response = auth_client.post("/api/settings/language", data=json.dumps(payload), content_type="application/json")
...
class TestSettingsIntegration:
    ...
    def test_settings_affect_brute_force_protection(self, client, db):
        ...
        if setting:
            ...
            setting = AdminSettings(setting_key="login_max_attempts", setting_value="3")
            db.session.add(setting)
        db.session.commit()
    ...
    def test_settings_affect_lock_duration(self, client, db):
        ...
        if setting:
            ...
            setting = AdminSettings(setting_key="login_lock_duration_minutes", setting_value="1")
            db.session.add(setting)
        db.session.commit()
