...
from app.models.models import Capability, ConnectionAccount, McpService, Service


@pytest.fixture
def prompt_capability(db):
    """Create a prompt-type capability with template"""
    mcp_service = McpService(name="Prompt Service", identifier="prompt-service", routing_type="subdomain")
    db.session.add(mcp_service)
    db.session.flush()

    service = Service(
        mcp_service_id=mcp_service.id, name="Prompt App", service_type="api", mcp_url="https://api.example.com"
    )
...
def tool_capability(db):
    ...
    mcp_service = McpService(name="Tool Service", identifier="tool-service", routing_type="subdomain")
    db.session.add(mcp_service)
    db.session.flush()

    service = Service(
        mcp_service_id=mcp_service.id, name="Tool App", service_type="api", mcp_url="https://api.example.com"
    )
    db.session.add(service)
    db.session.flush()
...
def test_create_prompt_capability(db):
    ...
    service = Service(
        mcp_service_id=mcp_service.id, name="Test App", service_type="api", mcp_url="https://api.example.com"
    )
    db.session.add(service)
    db.session.flush()
...
def test_prompts_list_returns_only_prompt_type(client, db, prompt_capability, tool_capability):
    ...
    account = ConnectionAccount(name="test_account", bearer_token="test_token_123")
    ...
    capabilities = Capability.query.filter_by(capability_type="prompt").all()
...
def test_prompt_body_params_defines_variables(prompt_capability):
    ...
    for _key, schema in properties.items():
...
def test_multiple_prompt_capabilities(db):
    ...
    mcp_service = McpService(name="Multi Prompt Service", identifier="multi-prompt", routing_type="subdomain")
    db.session.add(mcp_service)
    db.session.flush()

    service = Service(
        mcp_service_id=mcp_service.id, name="Multi Prompt App", service_type="api", mcp_url="https://api.example.com"
    )
    db.session.add(service)
    db.session.flush()
    ...
    # Query all prompts
    all_prompts = Capability.query.filter_by(app_id=service.id, capability_type="prompt").all()
