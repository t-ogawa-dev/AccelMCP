...
from app.models.models import Capability, McpService, Resource, Service
...
class TestResourceCapabilityCreation:
    ...
    def test_create_resource_capability_with_existing_resource(
        self, auth_client, db, sample_service_for_prompt, sample_global_resource
    ):
...
class TestPromptCapabilityUpdate:
    ...
    def test_update_prompt_capability_without_resource_record(self, auth_client, db, sample_service_for_prompt):
        ...
        response = auth_client.put(
            f"/api/capabilities/{prompt_cap.id}", data=json.dumps(data), content_type="application/json"
        )
    ...
    def test_update_resource_capability_creates_resource_record(self, auth_client, db, sample_service_for_prompt):
        ...
        response = auth_client.put(
            f"/api/capabilities/{resource_cap.id}", data=json.dumps(data), content_type="application/json"
        )
...
class TestPromptsGetEndpoint:
    ...
    def test_prompts_get_with_template_content(
        self, auth_client, db, sample_service_for_prompt, sample_connection_account
    ):
        ...
        mcp_request = {
            ...
            "id": 1,
            "method": "prompts/get",
            "params": {"name": "test_prompt_with_content"},
        }
    ...
    def test_prompts_get_with_resource_fallback(
        self, auth_client, db, sample_service_for_prompt, sample_global_resource, sample_connection_account
    ):
        ...
        mcp_request = {
            ...
            "id": 1,
            "method": "prompts/get",
            "params": {"name": "test_prompt_with_resource"},
        }
...
class TestCapabilityListAPI:
    ...
    def test_capability_list_includes_all_types(
        self, auth_client, db, sample_service_for_prompt, sample_global_resource
    ):
    ...
    def test_capability_detail_returns_correct_fields_for_resource(
        self, auth_client, db, sample_service_for_prompt, sample_global_resource
    ):
