...
def test_create_capability_via_api_integration(client, db):
    ...
    # Create Service
    service = Service(
        mcp_service_id=mcp_service.id, name="Create Test App", service_type="api", mcp_url="https://api.example.com"
    )
    db.session.add(service)
    db.session.commit()
    ...
    response = client.post(
        f"/api/apps/{service.id}/capabilities",
        json={
            "name": "new_capability",
            "capability_type": "tool",
...
def test_update_capability_via_api_integration(client, db):
    ...
    service = Service(
        mcp_service_id=mcp_service.id, name="Update Test App", service_type="api", mcp_url="https://api.example.com"
    )
    db.session.add(service)
    db.session.flush()
    ...
    response = client.put(
        ...
        json={"name": "updated_capability", "timeout_seconds": 120, "description": "New description"},
    )
...
def test_capability_to_dict_with_service_relationship(db):
    ...
    # Create MCP Service
    mcp_service = McpService(name="To Dict Test Service", identifier="to-dict-test", routing_type="subdomain")
    db.session.add(mcp_service)
    db.session.flush()

    # Create Service
    service = Service(
        mcp_service_id=mcp_service.id, name="To Dict Test App", service_type="api", mcp_url="https://api.example.com"
    )
    db.session.add(service)
    db.session.flush()
