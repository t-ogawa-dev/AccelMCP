...
class TestMcpServiceModel:
    ...
    def test_mcp_service_to_dict(self, db):
        ...
        mcp_service = McpService(
            name="Dict Test", identifier="dict-test", routing_type="path", description="Test", is_enabled=False
        )
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_mcp_service_with_apps(self, db):
        ...
        mcp_service = McpService(name="Service with Apps", identifier="service-apps", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()

        # Create apps associated with this MCP service
        app1 = Service(name="App 1", service_type="api", common_headers="{}", mcp_service_id=mcp_service.id)
        app2 = Service(name="App 2", service_type="mcp", common_headers="{}", mcp_service_id=mcp_service.id)
...
class TestMcpServiceAPI:
    ...
    def test_create_mcp_service(self, auth_client):
        ...
        response = auth_client.post("/api/mcp-services", data=json.dumps(payload), content_type="application/json")
    ...
    def test_get_mcp_service_detail(self, auth_client, db):
        ...
        mcp_service = McpService(name="Detail Service", identifier="detail-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_update_mcp_service(self, auth_client, db):
        ...
        mcp_service = McpService(name="Update Service", identifier="update-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()

        payload = {"name": "Updated MCP Service", "description": "Updated description"}
        response = auth_client.put(
            f"/api/mcp-services/{mcp_service.id}", data=json.dumps(payload), content_type="application/json"
        )
    ...
    def test_toggle_mcp_service(self, auth_client, db):
        ...
        mcp_service = McpService(
            name="Toggle Service", identifier="toggle-service", routing_type="subdomain", is_enabled=True
        )
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_delete_mcp_service(self, auth_client, db):
        ...
        mcp_service = McpService(name="Delete Service", identifier="delete-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_duplicate_identifier(self, auth_client, db):
        ...
        payload1 = {"name": "Original", "identifier": "duplicate", "routing_type": "subdomain"}
        auth_client.post("/api/mcp-services", data=json.dumps(payload1), content_type="application/json")

        # Try to create duplicate
        payload = {"name": "Duplicate", "identifier": "duplicate", "routing_type": "subdomain"}
        response = auth_client.post("/api/mcp-services", data=json.dumps(payload), content_type="application/json")
...
"""
        ...
        response = auth_client.post("/api/mcp-services/import", data=yaml_data, content_type="application/x-yaml")
    ...
    def test_import_mcp_service_identifier_collision(self, auth_client, db):
        ...
        existing = McpService(name="Existing", identifier="duplicate-id", routing_type="subdomain")
...
"""
        ...
        response = auth_client.post("/api/mcp-services/import", data=invalid_yaml, content_type="application/x-yaml")
...
class TestMcpServiceAppsAPI:
    ...
    def test_get_mcp_service_apps(self, auth_client, db):
        ...
        mcp_service = McpService(name="Service", identifier="service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_create_app_for_mcp_service(self, auth_client, db):
        ...
        payload = {"name": "New App", "service_type": "api", "common_headers": "{}"}
        response = auth_client.post(
            f"/api/mcp-services/{mcp_service.id}/apps", data=json.dumps(payload), content_type="application/json"
        )
...
class TestMcpServiceRouting:
    ...
    def test_subdomain_routing(self, db):
        ...
        mcp_service = McpService(name="Subdomain Service", identifier="sub-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_path_routing(self, db):
        ...
        mcp_service = McpService(name="Path Service", identifier="path-service", routing_type="path")
        db.session.add(mcp_service)
        db.session.commit()
...
class TestMcpServiceAccessControl:
    ...
    def test_public_access(self, db):
        ...
        mcp_service = McpService(
            name="Public Service", identifier="public", routing_type="subdomain", access_control="public"
        )
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_restricted_access(self, db):
        ...
        mcp_service = McpService(
            name="Restricted Service", identifier="restricted", routing_type="subdomain", access_control="restricted"
        )
        db.session.add(mcp_service)
        db.session.commit()
...
class TestMcpServiceViews:
    ...
    def test_mcp_service_detail_view(self, auth_client, db):
        ...
        mcp_service = McpService(name="View Service", identifier="view-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_mcp_service_edit_view(self, auth_client, db):
        ...
        mcp_service = McpService(name="Edit Service", identifier="edit-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
    ...
    def test_mcp_service_apps_view(self, auth_client, db):
        ...
        mcp_service = McpService(name="Apps Service", identifier="apps-service", routing_type="subdomain")
        db.session.add(mcp_service)
        db.session.commit()
