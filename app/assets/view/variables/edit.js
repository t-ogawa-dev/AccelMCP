...
async function loadVariable() {
    ...
    // Set source_type radio button
    if (variable.source_type === 'env') {
        document.getElementById('source-env').checked = true;
        document.getElementById('env_var_name').value = variable.env_var_name || '';
    } else {
        document.getElementById('source-value').checked = true;
...
document.getElementById('variable-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            await modal.error(t('variable_update_failed') + ': ' + (error.error || t('error_unknown')));
        }
    } catch (error) {
        await modal.error(t('variable_update_failed') + ': ' + error.message);
    }
});
